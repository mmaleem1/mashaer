...
async function positionPointerForGizmoDrag(page, point, frameCount = 3) {
  await page.evaluate(() => {
    window.__mashaer.dataManager.layers.get('cctv').module
      .setParams({ calibrationMode: false });
  });
  await page.mouse.move(point.x, point.y);
  return page.evaluate(async (count) => {
    const mashaer = window.__mashaer;
    mashaer.dataManager.layers.get('cctv').module.setParams({ calibrationMode: true });
...
async function main() {
  ...
  try {
    ...
    const activeIdBeforeActivation = await page.evaluate(() => window.__mashaer.dataManager.layers.get('cctv').module.getUIState().activeCameraId);
    ...
    const activeId = await page.evaluate(() => window.__mashaer.dataManager.layers.get('cctv').module.getUIState().activeCameraId);
    ...
    await page.evaluate((id) => {
      window.__mashaer.dataManager.layers.get('cctv').module.setParams({ selectedCameraId: id });
    }, activeId);
    await sleep(400);
    ...
    const baseStoreEmpty = await page.evaluate((camId) => {
      const raw = localStorage.getItem('mashaer.cctv.calibration.v2');
    ...
    const clickEvidenceSetup = await page.evaluate(() => {
      const mashaer = window.__mashaer;
      const viewer = mashaer.viewer;
      const mod = mashaer.dataManager.layers.get('cctv').module;
      const snapshotPose = () => ({
      ...
      window.addEventListener('mashaer:cctv-request-focus', evidence.onFocus);
      evidence.dispose = () => {
        evidence.unsubscribe?.();
        window.removeEventListener('mashaer:cctv-request-focus', evidence.onFocus);
      };
    ...
    const siblingClick = await page.evaluate(() => {
      ...
      if (window.__qaCctvSiblingOwner) {
        mashaer.viewer.entities.remove(window.__qaCctvSiblingOwner);
    ...
    const firstEmptyClick = await page.evaluate(() => {
      const evidence = window.__qaCctvClickEvidence;
      const state = window.__mashaer.dataManager.layers.get('cctv').module.getUIState();
      return {
    ...
    const countVolumes = () => page.evaluate(() => {
      ...
      let n = 0;
      for (let i = 0; i < prims.length; i++) {
        if (prims.get(i) && prims.get(i)._mashaerViewshed) n += 1;
      }
      return n;
    });
    ...
    await page.evaluate(() => {
      ...
      for (let i = 0; i < prims.length; i++) {
        if (prims.get(i) && prims.get(i)._mashaerViewshed) window.__qaViewshedRefs.push(prims.get(i));
      }
    });
    await sleep(8000);
    const viewshedIdle = await page.evaluate(() => {
      ...
      const now = [];
      for (let i = 0; i < prims.length; i++) {
        if (prims.get(i) && prims.get(i)._mashaerViewshed) now.push(prims.get(i));
      }
