...
async function main() {
  ...
  try {
    ...
    await page.waitForFunction(() => window.__mashaer?.dataManager, { timeout: 60_000 });
    await page.waitForFunction(() => window.__mashaer?.styleManager?._dataManager?.layers?.has('radio'), { timeout: 60_000 });
    ...
    await page.evaluate(() => {
      window.__mashaer.viewer.camera.cancelFlight();
      window.__mashaer.styleManager.setPanelCollapsed('pp-toggles', true);
      window.__mashaer.styleManager.setPanelCollapsed('cctv-panel', true);
      window.__mashaer.styleManager.setPanelCollapsed('radio-panel', true);
      window.__mashaer.styleManager.setPanelCollapsed('global-context-panel', true);
    });
    ...
    const initialDisclosure = await page.evaluate(() => {
      ...
      const enabledBefore = window.__mashaer.dataManager.isEnabled('radio');
      button.click();
      return {
        enabledBefore,
        enabledAfterDisclosure: window.__mashaer.dataManager.isEnabled('radio'),
    ...
    await page.waitForFunction(() => {
      const state = window.__mashaer.dataManager.layers.get('radio').module.getUIState();
      return state.enabled && !state.loading && state.stationCount === 750;
    });
    const enableElapsedMs = Date.now() - enableStartedAt;
    const initial = await page.evaluate(() => {
      const module = window.__mashaer.dataManager.layers.get('radio').module;
      ...
      return {
        ...
        tunerCount: window.__mashaer.styleManager._radioTunerStations.length,
    ...
    const uncertainPresentation = await page.evaluate(async () => {
      ...
      const style = mashaer.styleManager;
      const entry = mashaer.dataManager.layers.get('radio');
      ...
      const camera = mashaer.viewer.camera;
      ...
      mashaer.dataManager._refreshTogglePanel();
      ...
      const result = {
        ...
        markerSourceVisible: mashaer.viewer.dataSources.getByName('Radio stations')[0]?.show,
    ...
    await page.evaluate(() => {
      const mashaer = window.__mashaer;
      const module = mashaer.dataManager.layers.get('radio').module;
      ...
      window.__qaRadioDisablePromise = mashaer.dataManager.setEnabled('radio', false, { origin: 'qa-setup' });
    });
    await page.waitForFunction(() => window.__qaRadioDisableStarted === true);
    const disablingPresentation = await page.evaluate(() => ({
      manager: window.__mashaer.dataManager.getLayerLifecycleState('radio'),
      presentationActive: window.__mashaer.dataManager.layers.get('radio').module.getUIState().presentationActive,
      markerSourceVisible: window.__mashaer.viewer.dataSources.getByName('Radio stations')[0]?.show,
    ...
    const explicitRevealBefore = await page.evaluate(async () => {
      const mashaer = window.__mashaer;
      mashaer.styleManager.setPanelCollapsed('global-context-panel', false);
      mashaer.styleManager.setPanelCollapsed('radio-panel', false);
      await new Promise((resolve) => requestAnimationFrame(() => requestAnimationFrame(resolve)));
      ...
      const camera = mashaer.viewer.camera.positionWC;
      return {
    ...
    await page.evaluate(() => {
      const radio = window.__mashaer.dataManager.layers.get('radio').module;
    ...
    await page.waitForFunction(() => (
      window.__mashaer.dataManager.getLayerLifecycleState('radio')?.lifecycleState === 'enabling'
    ));
    ...
    await page.waitForFunction(() => window.__mashaer.dataManager.isEnabled('radio'));
    ...
    const explicitRevealAfter = await page.evaluate(() => {
      ...
      return {
        ...
        plays: window.__qaRadioPlayCalls.length,
        audioState: mashaer.dataManager.layers.get('radio').module.getUIState().audioState,
      };
    });
    ...
    const userFacingFailures = await page.evaluate(async () => {
      const mashaer = window.__mashaer;
      const styleManager = mashaer.styleManager;
      const dataManager = mashaer.dataManager;
      const unhandled = [];
      ...
      try {
        ...
        for (const outcome of ['false', 'reject']) {
          ...
          window.__mashaerQaRegisterLayer(dataManager, qaLayer);
          try {
            ...
            styleManager._showToast = originalShowToast;
            await window.__mashaerQaUnregisterLayer(dataManager, layerId);
          }
        }
        ...
        await pendingAction;
        styleManager._showToast = originalShowToast;
        await window.__mashaerQaUnregisterLayer(dataManager, unrelatedLayerId);
    ...
    await page.evaluate(async () => {
      await window.__mashaer.dataManager.layers.get('radio').module.update();
    });
    await page.select('#radio-filter', 'all');
    const allPalette = await page.evaluate(() => {
      const source = Array.from({ length: window.__mashaer.viewer.dataSources.length }, (_, index) => (
        window.__mashaer.viewer.dataSources.get(index)
      )).find((item) => item.name === 'Radio stations');
    ...
    const cockpitCompactControls = await page.evaluate(async () => {
      ...
      window.__mashaer.styleManager._syncLeftPanelAdaptiveLayout();
      window.__mashaer.styleManager.cockpitView.syncSignalLayout();
      ...
      const toggleBefore = toggleLayerId
        ? window.__mashaer.dataManager.isEnabled(toggleLayerId) : null;
      toggleButton?.click();
      await waitForLayout();
      ...
      const toggleRestored = toggleLayerId
        ? window.__mashaer.dataManager.isEnabled(toggleLayerId) === toggleBefore : false;
    ...
    const firstPlayPrecondition = await page.evaluate(() => (
      window.__mashaer.dataManager.layers.get('radio').module.getUIState().selected?.id || null
    ));
    ...
    await page.evaluate(() => {
      const camera = window.__mashaer.viewer.camera;
    ...
    const postMicroDragPlayback = await page.evaluate(async () => {
      const mod = window.__mashaer.dataManager.layers.get('radio').module;
    ...
    const newsClusterTarget = await page.evaluate(() => {
      const mashaer = window.__mashaer;
      const source = Array.from({ length: mashaer.viewer.dataSources.length }, (_, index) => mashaer.viewer.dataSources.get(index))
        .find((item) => item.name === 'Radio stations');
      const points = source.clustering._clusterPointCollection;
      for (let index = 0; index < (points?.length || 0); index += 1) {
        const point = points.get(index);
        if (!point.show || !Array.isArray(point.id) || !point.id.length) continue;
        const canvasPoint = mashaer.viewer.scene.cartesianToCanvasCoordinates(point.position);
    ...
    const newsClusterClick = await page.evaluate(() => {
      const mashaer = window.__mashaer;
      const state = mashaer.dataManager.layers.get('radio').module.getUIState();
      mashaer.dataManager.layers.get('radio').module.stopPlayback();
    ...
    const compactVolume = await page.evaluate(() => ({
      volume: window.__mashaer.dataManager.layers.get('radio').module.getUIState().volume,
    ...
    const voicePause = await page.evaluate(async () => {
      ...
      await window.__mashaer.dataManager.layers.get('radio').module.play();
      const startedAt = performance.now();
      window.__mashaer.voiceCommands.setStatus('connecting', 'QA voice connect');
      const state = window.__mashaer.dataManager.layers.get('radio').module.getUIState();
      return {
        ...state,
    ...
    const idleVoice = await page.evaluate(() => {
      const voice = window.__mashaer.voiceCommands;
      voice.setMicrophoneEnabled(true);
      voice.setStatus('listening', 'QA standby');
      voice.setVoiceSpeaker('idle');
      return window.__mashaer.dataManager.layers.get('radio').module.getUIState();
    });
    check(
    ...
    if (manualPlayTarget.visible && manualPlayTarget.enabled) {
      ...
      await page.waitForFunction(() => (
        window.__mashaer.dataManager.layers.get('radio').module.getUIState().audioState === 'playing'
        && window.__mashaer.voiceCommands.status === 'idle'
      ));
    }
    const manualTakeover = await page.evaluate(() => ({
      audioState: window.__mashaer.dataManager.layers.get('radio').module.getUIState().audioState,
      voiceStatus: window.__mashaer.voiceCommands.status,
      voiceDucked: window.__mashaer.dataManager.layers.get('radio').module.getUIState().voiceDucked,
    }));
    check(
    ...
    await page.evaluate(() => window.__mashaer.voiceCommands.setStatus('listening', 'QA standby'));
    ...
    await page.evaluate(() => window.__mashaer.voiceCommands.setStatus('idle', 'QA complete'));
    await page.evaluate(async () => {
      const manager = window.__mashaer.styleManager;
    ...
    const absoluteTuner = await page.evaluate(async () => {
      const mashaer = window.__mashaer;
      const style = mashaer.styleManager;
      const radio = mashaer.dataManager.layers.get('radio').module;
      const slider = document.getElementById('radio-tuner-slider');
      ...
      const snapshot = () => {
        const markerId = mashaer.viewer.entities.values
    ...
    const tunerCancelRestore = await page.evaluate(() => {
      ...
      const previewBeforeCancel = mashaer.viewer.entities.values
        .find((entity) => String(entity.id).startsWith('radio:selected:'))?.id || null;
      ...
      const selectedEntity = () => mashaer.viewer.entities.values
    ...
    const tunerRefreshTarget = await page.evaluate(() => {
      const mashaer = window.__mashaer;
      const slider = document.getElementById('radio-tuner-slider');
      const stations = mashaer.styleManager._radioTunerStations;
      const targetIndex = 1;
      ...
      return {
        ...
        clientX,
        clientY: rect.top + rect.height / 2,
        generation: mashaer.dataManager.layers.get('radio').module.getUIState().tuningCatalogGeneration,
        plays: window.__qaRadioPlayCalls.length,
      };
    });
    ...
    const tunerRefreshRelease = await page.evaluate((target) => {
      ...
      const frozenGeneration = mashaer.dataManager.layers.get('radio').module.getUIState().tuningCatalogGeneration;
    ...
    const tunerDirectRelease = await page.evaluate(async () => {
      ...
      const targetId = mashaer.styleManager._radioTunerStations[targetIndex]?.id || null;
      const rect = slider.getBoundingClientRect();
    ...
    const tunerCommitTarget = await page.evaluate(() => {
      ...
      const station = mashaer.styleManager._radioTunerStations[targetIndex];
      const rect = slider.getBoundingClientRect();
      ...
      return {
        id: station.id,
        targetIndex,
        count: mashaer.styleManager._radioTunerStations.length,
        frozenSignature: mashaer.styleManager._radioTunerBandSignature,
        startX: rect.left + 7 + (rect.width - 14) * currentRatio,
        targetX: rect.left + 7 + (rect.width - 14) * targetIndex
          / Math.max(1, mashaer.styleManager._radioTunerStations.length - 1),
    ...
    const tunerCommit = await page.evaluate((target) => {
      ...
      return {
        ...
        signatureStable: mashaer.styleManager._radioTunerBandSignature === target.frozenSignature,
        selectedIndex: mashaer.styleManager._radioTunerStations.findIndex((item) => item.id === state.selected?.id),
        sliderSlot: Number(slider.value),
        ratio: Number(document.getElementById('radio-tuner').style.getPropertyValue('--radio-tuner-ratio')),
        pinned: mashaer.styleManager._radioTunerBandPinnedForNavigation,
      };
    }, tunerCommitTarget);
    check(
    ...
    const microDragStart = await page.evaluate(() => {
      const style = window.__mashaer.styleManager;
      const slider = document.getElementById('radio-tuner-slider');
      const rect = slider.getBoundingClientRect();
    ...
    const viewportTuner = await page.evaluate(async (selectedId) => {
      ...
      const current = mashaer.viewer.camera.positionCartographic;
      const oppositeLongitude = ((current.longitude + 2 * Math.PI) % (2 * Math.PI)) - Math.PI;
      mashaer.viewer.canvas.dispatchEvent(new WheelEvent('wheel', { bubbles: true, deltaY: 1 }));
      mashaer.viewer.camera.setView({
        destination: mashaer.viewer.scene.globe.ellipsoid.cartographicToCartesian({
    ...
    const nextNeedleBefore = await page.evaluate(() => {
      const style = window.__mashaer.styleManager;
      const selectedId = window.__mashaer.dataManager.layers.get('radio').module.getUIState().selected?.id;
    ...
    const filteredTunerCenter = await page.evaluate(() => {
      ...
      const preview = {
        ...
        slot: Number(slider.value),
        ratio: Number(document.getElementById('radio-tuner').style.getPropertyValue('--radio-tuner-ratio')),
        markerId: mashaer.viewer.entities.values
          .find((entity) => String(entity.id).startsWith('radio:selected:'))?.id || null,
    ...
    const fullPoolNavigation = await page.evaluate(() => {
      const mashaer = window.__mashaer;
      const radio = mashaer.dataManager.layers.get('radio').module;
      const pool = mashaer.styleManager._radioTunerPool;
    ...
    const failedTuner = await page.evaluate(async () => {
      ...
      const slider = document.getElementById('radio-tuner-slider');
      const rect = slider.getBoundingClientRect();
      const count = mashaer.styleManager._radioTunerStations.length;
      ...
      const failed = mashaer.dataManager.layers.get('radio').module.getUIState();
      mashaer.dataManager.layers.get('radio').module.stopPlayback();
      const stopped = mashaer.dataManager.layers.get('radio').module.getUIState();
      return {
        failed: {
    ...
    if (markerTarget?.found) {
      ...
      await page.waitForFunction(
        (id) => window.__mashaer.dataManager.layers.get('radio').module.getUIState().selected?.id === id,
        {},
        markerTarget.id,
      );
      const offsetSelected = await page.evaluate(() => window.__mashaer.dataManager.layers.get('radio').module.getUIState().selected?.id);
    ...
    await page.evaluate(() => {
      ...
      radio.stopPlayback();
      Array.from({ length: mashaer.viewer.dataSources.length }, (_, index) => mashaer.viewer.dataSources.get(index))
      ...
      mashaer.dataManager.layers.get('flights').enabled = true;
    });
    await sleep(500);
    await page.evaluate(() => window.__mashaer.styleManager.setPanelCollapsed('global-context-panel', true));
    await page.evaluate(() => {
      const button = document.getElementById('context-radio-toggle-btn');
      if (button.getAttribute('aria-expanded') !== 'true') button.click();
    });
    ...
    await page.evaluate(() => {
      window.__qaRadioCameraHeight = window.__mashaer.viewer.camera.positionCartographic.height;
    });
    ...
    await page.waitForFunction(() => window.__mashaer.dataManager.layers.get('radio').module.getUIState().audioState === 'playing');
    const playing = await page.evaluate(async () => {
      const radio = window.__mashaer.dataManager.layers.get('radio').module;
      const viewer = window.__mashaer.viewer;
      const selectedEntity = window.__mashaer.viewer.entities.values
      ...
      regularMarkerSize: Array.from({ length: window.__mashaer.viewer.dataSources.length }, (_, index) => window.__mashaer.viewer.dataSources.get(index))
    ...
    const trackedRadioControls = await page.evaluate(async () => {
      ...
      const run = async (ownerPrefix, buttonId) => {
        ...
        owner.mashaerTrackedId = `${ownerPrefix}:qa-radio-camera-owner`;
        ...
        const result = {
          ...
          trackedId: viewer.trackedEntity?.mashaerTrackedId || null,
        };
    ...
    const delayedTrackingOwner = await page.evaluate(() => {
      ...
      owner.mashaerTrackedId = 'flights:qa-delayed-radio-fallback-owner';
      viewer.trackedEntity = owner;
      window.__qaDelayedRadioTrackingOwner = owner;
      window.__qaRejectRadioPlay();
      return owner.mashaerTrackedId;
    });
    await page.waitForFunction(() => window.__mashaer.dataManager.layers.get('radio').module.getUIState().audioState === 'playing');
    const delayedFallbackOwnership = await page.evaluate((beforeFlyToCalls) => {
      ...
      const result = {
        ...
        trackedIdentityPreserved: viewer.trackedEntity === owner,
        trackedId: viewer.trackedEntity?.mashaerTrackedId || null,
        selectedId: window.__mashaer.dataManager.layers.get('radio').module.getUIState().selected?.id || null,
      };
      viewer.trackedEntity = undefined;
      viewer.entities.remove(owner);
    ...
    await page.evaluate(async () => {
      const viewer = window.__mashaer.viewer;
      window.__mashaer.styleManager.toggleCleanView(true);
      viewer.resize();
      viewer.scene.requestRender();
      await new Promise((resolve) => requestAnimationFrame(() => requestAnimationFrame(resolve)));
    });
    ...
    await page.evaluate(() => {
      const viewer = window.__mashaer.viewer;
      window.__mashaer.styleManager.toggleCleanView(false);
      viewer.resize();
      viewer.scene.requestRender();
    });
    await page.evaluate(() => {
    ...
    await page.evaluate(() => {
      window.__qaRadioFlyToCalls = [];
      window.__mashaer.dataManager.layers.get('flights').enabled = false;
    });
    ...
    const radioOnlyFocusCalls = await page.evaluate(() => {
      ...
      const heights = window.__qaRadioFlyToCalls.map((options) => (
        window.__mashaer.viewer.scene.globe.ellipsoid.cartesianToCartographic(options.destination).height
      ));
      window.__mashaer.viewer.camera.flyTo = window.__qaRadioOriginalFlyTo;
    ...
    const stagedGlobeNavigation = await page.evaluate(async () => {
      ...
      owner.mashaerTrackedId = 'flights:qa-radio-recenter-stage-owner';
      viewer.trackedEntity = owner;
      completeFlight(0);
    ...
    await page.evaluate(() => {
      window.__mashaer.styleManager.setPanelCollapsed('radio-panel', true);
      window.__mashaer.styleManager.setPanelCollapsed('global-context-panel', false);
    });
    const companion = await page.evaluate(() => ({
      radioEnabled: window.__mashaer.dataManager.isEnabled('radio'),
      state: window.__mashaer.dataManager.layers.get('radio').module.getUIState().audioState,
      radioCollapsed: document.getElementById('radio-panel').classList.contains('collapsed'),
      contextCollapsed: document.getElementById('global-context-panel').classList.contains('collapsed'),
    ...
    const expandedContextRadioAfter = await page.evaluate(() => {
      const mashaer = window.__mashaer;
      mashaer.styleManager._renderRadioState(mashaer.dataManager.layers.get('radio').module.getUIState());
      const launcher = document.getElementById('context-radio-toggle-btn');
      const radio = document.getElementById('radio-panel');
      const scroller = document.querySelector('#global-context-panel .global-context-panel-inner');
      const camera = mashaer.viewer.camera.positionWC;
      return {
        ...
        camera: { x: camera.x, y: camera.y, z: camera.z },
        plays: window.__qaRadioPlayCalls.length,
        state: mashaer.dataManager.layers.get('radio').module.getUIState().audioState,
      };
    });
    ...
    const filtered = await page.evaluate(() => window.__mashaer.dataManager.layers.get('radio').module.getUIState());
    ...
    await page.evaluate(async () => {
      const mashaer = window.__mashaer;
      await mashaer.dataManager.setEnabled('radio', false);
      await mashaer.dataManager.setEnabled('radio', true);
    });
    const restored = await page.evaluate(() => ({
      state: window.__mashaer.dataManager.layers.get('radio').module.getUIState(),
      plays: window.__qaRadioPlayCalls.length,
      radioCollapsed: document.getElementById('radio-panel').classList.contains('collapsed'),
    }));
    check(
    ...
    await page.evaluate(() => window.__mashaer.styleManager.setPanelCollapsed('radio-panel', false));
    await sleep(350);
