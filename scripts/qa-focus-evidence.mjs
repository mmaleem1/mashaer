...
async function readTileReadiness(page) {
  return page.evaluate(() => {
    ...
    const controller = mashaer.mapStackController;
    const activeStack = controller?.getActiveId?.() || null;
    const tileset = controller?.googleTileset || mashaer.tileset || null;
  ...
  const result = await page.evaluate(async (timeoutMs) => {
    ...
    try {
      while (Date.now() < deadline) {
        mashaer.viewer.scene.render(mashaer.viewer.clock.currentTime);
...
async function capture(page, scenario, frame) {
  ...
  const dataUrl = await page.evaluate(() => (
    window.__mashaer.viewer.scene.canvas.toDataURL('image/png')
  ));
...
async function takeFrameClock(page) {
  const result = await page.evaluate(() => {
    const seam = window.__mashaer.dataManager.layers.get('flights').module.__focusEvidence;
...
async function advanceEvidenceFrame(page, deltaMs = FRAME_MS) {
  return page.evaluate((stepMs) => {
    ...
    const seam = mashaer.dataManager.layers.get('flights').module.__focusEvidence;
...
async function setBasemap(page, stackId) {
  const result = await page.evaluate(async (id) => (
    window.__mashaer.styleManager.setMapStack(id)
  ), stackId);
  if (!result?.ok) {
...
async function enableLayer(page, layerId) {
  await page.evaluate(async (id) => {
    await window.__mashaer.dataManager.setEnabled(id, true);
  }, layerId);
}
...
async function injectAndTrackTarget(page, extraAircraft = [], target = TARGET) {
  ...
  await page.waitForFunction(
    () => Boolean(window.__mashaer.viewer.trackedEntity),
    { timeout: 10_000 },
  );
...
async function readTrackedBasis(page) {
  return page.evaluate((targetId) => {
    ...
    const canvasHeight = mashaer.viewer.scene.canvas.clientHeight;
...
async function flightSnapshot(page) {
  return page.evaluate(() => (
    window.__mashaer.dataManager.layers.get('flights').module.__focusEvidence.snapshot()
  ));
}

async function setTuning(page, params) {
  return page.evaluate((next) => (
    window.__mashaer.dataManager.layers.get('flights').module.__focusEvidence.setTuning(next)
  ), params);
}
...
async function vesselRowsAroundTarget(page) {
  return page.evaluate(() => {
    ...
    const Cartographic = mashaer.viewer.camera.positionCartographic.constructor;
...
async function runHorizonBand(page, effectiveParams) {
  ...
  await page.evaluate((records) => {
    window.__mashaer.dataManager.layers.get('flights')
