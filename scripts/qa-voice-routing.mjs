 ...
 *    Drives window.__mashaerVoiceCommands.runner(toolName, args) in a headless
 *    page against the dev server and asserts world state via __mashaer /
 *    __mashaerAnnotations (camera altitude bands, annotation counts, route
...
async function runRoutingLayer() {
  ...
  const logDir = path.join(ROOT, '.mashaer-logs', 'qa-voice-routing');
...
async function runBehaviorLayer() {
  ...
  try {
    ...
    const run = (name, args) => page.evaluate(
      (n, a) => Promise.resolve(window.__mashaerVoiceCommands.runner(n, a))
        .catch((e) => ({ ok: false, error: String(e?.message || e), _threw: true })),
      name, args,
    );
    const camState = () => page.evaluate(() => {
      const c = window.__mashaer.viewer.camera;
    ...
    await settle(2500);
    let annoCount = await page.evaluate(() => window.__mashaerAnnotations?.list?.().length ?? window.__mashaerAnnotations?.count?.() ?? -1);
    ...
    const ownerTransfer = await page.evaluate(async () => {
      const app = window.__mashaer;
      const runner = window.__mashaerVoiceCommands?.runner;
    ...
    await page.waitForFunction(
      () => window.__mashaer.viewer.camera.positionCartographic.height / 1000 > 12000,
    ...
    await page.waitForFunction(
      () => window.__mashaer.viewer.camera.positionCartographic.height / 1000 < 900,
    ...
    for (let i = 0; i < 10 && !alpsOutline; i += 1) {
      ...
      alpsOutline = await page.evaluate(() => {
        ...
        const items = window.__mashaerAnnotations?.list?.() || [];
    ...
    const heading = () => page.evaluate(() => window.__mashaer.viewer.camera.heading * 180 / Math.PI);
    ...
    if (r?.ok) {
      ...
      const trackingReleased = await page.evaluate(() => !window.__mashaer.viewer.trackedEntity);
    ...
    if (trackId2 && (await run('track_entity', { query: trackId2, layerId: 'flights' }))?.ok) {
      ...
      await settle(6000);
      const stillTracked = await page.evaluate(() => Boolean(window.__mashaer.viewer.trackedEntity));
    ...
    await settle(800);
    annoCount = await page.evaluate(() => window.__mashaerAnnotations?.list?.().length ?? window.__mashaerAnnotations?.count?.() ?? -1);
    ...
    const contextEntered = await page.evaluate(
      () => window.__mashaer?.styleManager?.getContextModeState?.() || null,
    );
    ...
    const cockpitBefore = await page.evaluate(
      () => Boolean(window.__mashaer?.styleManager?.cockpitView?.active),
    );
    ...
    const awarenessFlights = await page.evaluate(() => {
      const snap = window.__mashaer?.dataManager?.layers
    ...
    // airliner.
    if (await page.evaluate(() => Boolean(window.__mashaer?.styleManager?.cockpitView?.active))) {
    ...
    const gated = await page.evaluate(
      () => window.__mashaer?.styleManager?.controlCockpit?.('enter') || null,
    );
