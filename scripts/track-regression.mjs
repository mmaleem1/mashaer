...
async function main() {
  ...
  try {
    ...
    page.on('console', (msg) => {
      ...
      if (process.env.MASHAER_TEST_VERBOSE) console.log(`    [page:${type}] ${text}`);
    });
    ...
    await page.evaluateOnNewDocument((synth, appOrigin) => {
      ...
      window.fetch = (input, init) => {
        ...
        if (isAppRequest && url.pathname === '/api/opensky') {
          ...
          const withheld = (() => {
            try { return window.sessionStorage.getItem('__mashaerWithhold') || ''; } catch { return ''; }
          })();
    ...
    const glbBackendCapable = glbControlStarted && await page.waitForFunction(() => {
      ...
      window.__mashaer.viewer.scene.requestRender();
    ...
    await evalPage(() => {
      window.__mashaer.viewer.scene.sampleHeight = () => undefined;
    });
    ...
    const enabled = await evalPage(async () => {
      ...
      await fl.update(window.__mashaer.viewer);
      await mil.update(window.__mashaer.viewer);
      return {
    ...
    const shareTracked = await evalPage(() => (
      window.__mashaer.dataManager.layers
        .get('flights').module.trackById('aaa001', { origin: 'user' })
    ));
    await page.waitForFunction(
      () => {
        const flights = window.__mashaer?.dataManager?.layers?.get('flights')?.module;
    ...
    await page.waitForFunction(
      () => window.__mashaer?.dataManager?.layers?.size >= 12,
      { timeout: 60000, polling: 200 },
    );
    await page.waitForFunction(
      () => {
        ...
        return manager?.isEnabled?.('flights')
          ...
          && window.__mashaer.viewer.trackedEntity?.mashaerTrackedId === 'flights:aaa001';
      },
      { timeout: 30000, polling: 100 },
    );
    const shareReload = await evalPage(() => {
      ...
      return {
        ...
        viewerTrackedId: window.__mashaer.viewer.trackedEntity?.mashaerTrackedId || null,
      };
    });
    record(
    ...
    await evalPage(() => localStorage.removeItem('mashaer:layer-state:v2'));
    ...
    await evalPage(() => window.sessionStorage.setItem('__mashaerWithhold', 'aaa001'));
    ...
    const pendingShare = await evalPage(() => ({
      tracked: window.__mashaer.dataManager.layers
    ...
    await evalPage(() => window.sessionStorage.removeItem('__mashaerWithhold'));
    await evalPage(async () => {
      const flights = window.__mashaer.dataManager.layers.get('flights').module;
      for (let i = 0; i < 3; i += 1) {
        await flights.update(window.__mashaer.viewer);
        await new Promise((resolve) => setTimeout(resolve, 600));
      }
    });
    await page.waitForFunction(
      () => window.__mashaer?.dataManager?.layers?.get('flights')?.module
    ...
    await evalPage(async () => {
      ...
      await manager.layers.get('military').module.update(window.__mashaer.viewer);
    });
    await evalPage(() => {
      window.__mashaer.viewer.scene.sampleHeight = () => undefined;
    });
    ...
    const runnerReady = await evalPage(() => typeof window.__mashaerVoiceCommands?.runner === 'function');
    if (!runnerReady) {
      ...
      const flightSelected = await evalPage(async () => {
        ...
        const result = await window.__mashaerVoiceCommands.runner('get_entity_context', { scope: 'selected' });
        return {
      ...
      const satelliteContext = await evalPage(async () => {
        ...
        const selected = await window.__mashaerVoiceCommands.runner('get_entity_context', { scope: 'selected' });
        // Switching subjects must move the slot, not stack a second orbit.
        satellites.trackById(20580, { origin: 'user' });
        await new Promise((resolve) => setTimeout(resolve, 500));
        const switched = await window.__mashaerVoiceCommands.runner('get_entity_context', { scope: 'selected' });
        const satelliteRecordIds = [...window.__mashaerContextStore.entities.values()]
        ...
        await new Promise((resolve) => setTimeout(resolve, 500));
        await satellites.update(window.__mashaer.viewer);
        await new Promise((resolve) => setTimeout(resolve, 600));
        const afterRebuild = await window.__mashaerVoiceCommands.runner('get_entity_context', { scope: 'selected' });
        ...
        const afterPartial = await window.__mashaerVoiceCommands.runner('get_entity_context', { scope: 'selected' });
        ...
        await satellites.update(window.__mashaer.viewer);
        await new Promise((resolve) => setTimeout(resolve, 800));
        const afterSubjectGone = await window.__mashaerVoiceCommands.runner('get_entity_context', { scope: 'selected' });
        window.__SYNTH.tle = fullTle;
        ...
        await new Promise((resolve) => setTimeout(resolve, 600));
        const denseSelected = await window.__mashaerVoiceCommands.runner('get_entity_context', { scope: 'selected' });
        window.__SYNTH.failDense = true;
        await satellites.update(window.__mashaer.viewer);
        await new Promise((resolve) => setTimeout(resolve, 1200));
        const afterDenseFailure = await window.__mashaerVoiceCommands.runner('get_entity_context', { scope: 'selected' });
        ...
        satellites.stopTracking?.();
        await new Promise((resolve) => setTimeout(resolve, 400));
        const released = await window.__mashaerVoiceCommands.runner('get_entity_context', { scope: 'selected' });
      ...
      const panelVsSpoken = await evalPage(async () => {
        ...
        if (button && !window.__mashaer.styleManager.getContextModeState?.().mode) button.click();
        for (let i = 0; i < 80; i += 1) {
          if (window.__mashaer.styleManager.getContextModeState?.().mode) break;
        ...
        const spoken = await window.__mashaerVoiceCommands.runner('analyst_query', {
      ...
      await evalPage(async () => {
        try { await window.__mashaerVoiceCommands.runner('set_context_mode', { mode: 'off' }); } catch {}
      });
      ...
      const manualContacts = await evalPage(async () => {
        ...
        const state = await window.__mashaerVoiceCommands.runner('get_current_view_state');
        return {
          internalMode: window.__mashaer.styleManager.getContextModeState?.().mode || null,
      ...
      const roundTrip = await evalPage(async (reported) => {
        const result = await window.__mashaerVoiceCommands.runner('set_context_mode', { mode: reported });
        const state = await window.__mashaerVoiceCommands.runner('get_current_view_state');
        return {
          ok: result?.ok ?? null,
      ...
      const contextOff = await evalPage(async () => {
        const result = await window.__mashaerVoiceCommands.runner('set_context_mode', { mode: 'off' });
        const state = await window.__mashaerVoiceCommands.runner('get_current_view_state');
        return {
          resultMode: result?.mode ?? null,
          stateMode: state?.context?.mode ?? null,
      ...
      const handoff = await evalPage(async () => {
        ...
        try {
          ...
          const analyst = await window.__mashaerVoiceCommands.runner('analyst_query', {
            layers: ['military'], scope: { kind: 'anywhere' }, limit: 10,
          });
          const item = (analyst?.items || []).find((entry) => entry.id === '6606') || null;
          const tracked = item
            ? await window.__mashaerVoiceCommands.runner('track_entity', { query: item.id, layerId: 'military' })
            : null;
          return {
      ...
      await evalPage(() => {
        window.__mashaer.dataManager.layers.get('flights').module.trackById('aaa001', { origin: 'user' });
      });
      await page.waitForFunction(
        () => window.__mashaer?.dataManager?.layers?.get('flights')?.module
          ?.getTrackedInfo?.()?.icao24 === 'aaa001',
    ...
    await evalPage(() => {
      window.__findTrackedModel = function () {
        ...
        const ent = window.__mashaer.viewer.trackedEntity;
        let tracked = null;
    ...
    if (!modelUp) {
      ...
      jitterResult = await sampleFrames(page, 30, (icao) => {
        ...
        const fl = dm.layers.get('flights').module;
        const ent = window.__mashaer.viewer.trackedEntity;
        const disp = ent && typeof ent.mashaerDisplayPosition === 'function' ? ent.mashaerDisplayPosition() : null;
    ...
    const heightBefore = await evalPage(() => window.__mashaer.viewer.camera.positionCartographic.height);
    ...
    const nowTrackingB = await evalPage((icao) => {
      const ti = window.__mashaer.dataManager.layers.get('flights').module.getTrackedInfo();
    ...
    if (!h1ClickPoint || !h1ClickPoint.via) {
      ...
      const h1HeightBefore = await evalPage(() => window.__mashaer.viewer.camera.positionCartographic.height);
    ...
    const m3Final = await evalPage(() => {
      const c = window.__mashaer.viewer.camera.positionCartographic;
      return {
    ...
    const staleCheck = await evalPage(async () => {
      ...
      const labelText = () => {
        ...
        const model = ent.mashaerLabelModel;
    ...
    try {
      g3dSetup = await evalPage(() => {
        ...
        const scene = mashaer.viewer.scene;
        ...
        window.__g3dPriorTilesLoadedDescriptor = mashaer.tileset
          ? Object.getOwnPropertyDescriptor(mashaer.tileset, 'tilesLoaded')
          : null;
        try {
          ...
          try {
            if (mashaer.tileset) {
              Object.defineProperty(mashaer.tileset, 'tilesLoaded', { value: true, configurable: true });
              tilesForced = mashaer.tileset.tilesLoaded === true;
            } else {
          ...
          attemptRollback('restore tilesLoaded seam', () => {
            if (!mashaer.tileset) return;
            const prior = window.__g3dPriorTilesLoadedDescriptor;
            if (prior) Object.defineProperty(mashaer.tileset, 'tilesLoaded', prior);
            else delete mashaer.tileset.tilesLoaded;
          });
      ...
      if (g3dModelsUp) {
        ...
        await evalPage(() => {
          window.__mashaer.dataManager.layers.get('flights').module.trackById('aaa077');
        });
        ...
        // `mashaerVisualPosition` is a SEPARATE accessor from `mashaerDisplayPosition` on
        ...
        if (g3dTrackedUp) {
          const trackedWeld = await evalPage(() => {
            ...
            const visual = typeof ent?.mashaerVisualPosition === 'function' ? ent.mashaerVisualPosition() : null;
            const display = typeof ent?.mashaerDisplayPosition === 'function' ? ent.mashaerDisplayPosition() : null;
            return {
              hasVisualAccessor: typeof ent?.mashaerVisualPosition === 'function',
          ...
          record('weld: mashaerDisplayPosition still reports the dead-reckoned position (anti-jitter intact)',
    ...
    const dfSetup = await evalPage(async () => {
      ...
      const mashaer = window.__mashaer;
      const v = mashaer.viewer;
      const fl = mashaer.dataManager.layers.get('flights').module;
      ...
      window.__dfPriorTilesLoadedDescriptor = mashaer.tileset
        ? Object.getOwnPropertyDescriptor(mashaer.tileset, 'tilesLoaded')
        : null;
      if (mashaer.tileset) {
        Object.defineProperty(mashaer.tileset, 'tilesLoaded', { value: true, configurable: true });
