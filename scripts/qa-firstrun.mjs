...
async function open(page, { hash = '', query = '', clearAll = true, clearSession = true, errorSink = null } = {}) {
  ...
  await page.waitForFunction(
    (sel) => !!window.__mashaer?.styleManager
...
const appState = (page) => page.evaluate(() => {
  ...
  return {
    ...
    durable: localStorage.getItem('mashaer:first-run-mission:v1'),
    session: sessionStorage.getItem('mashaer:first-run-mission-session:v1'),
    layerStateBlob: localStorage.getItem('mashaer:layer-state:v2'),
    allocation: localStorage.getItem('mashaer:detection-allocation:v1'),
  };
});
...
async function main() {
  ...
  try {
    ...
    await section('show-policy', async () => {
      ...
      const shareState = await page.evaluate(() => !!window.__mashaer?.styleManager?.hasShareState);
