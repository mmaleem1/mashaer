...
async function waitForCardCanvasInk(page, { timeoutMs = 12000 } = {}) {
  ...
  while (Date.now() < deadline) {
    await page.evaluate(() => window.__mashaer?.viewer?.scene?.requestRender?.());
    await sleep(150);
  ...
  try {
    ...
    console.log('\n(ii) CARDS — tactical card overlay ink at both LODs...');
    {
      const strongest = await page.evaluate(() => {
        const mod = window.__mashaer.dataManager.layers.get('local-firms').module;
    ...
    if (!(actionCards.present && actionCards.painted > 0 && beforeActions.count > 0)) {
      ...
      const prepared = await page.evaluate(() => {
        const mashaer = window.__mashaer;
        const mod = mashaer.dataManager.layers.get('local-firms').module;
        const target = mod.getDetectableObjects({ maxCount: 1 })[0];
        if (!target?.position) return null;

        const sentinel = mashaer.viewer.entities.add({
          id: 'qa-firms-prior-camera-owner',
          position: target.position,
        });
        mashaer.viewer.trackedEntity = sentinel;
        ...
        const proof = {
          ...
          request: null,
          generationBefore: mashaer.styleManager._navigationGeneration,
        };
        ...
        window.addEventListener('mashaer:world-request-focus', proof.onRequest);
        camera.flyToBoundingSphere = function qaFirmsObservedFlight(...args) {
          proof.flightCount += 1;
          proof.trackedAtFlight = mashaer.viewer.trackedEntity?.id || null;
      ...
      if (!prepared) {
        ...
        const proof = await page.evaluate(() => {
          ...
          const selectedEntityId = window.__mashaerContextStore?.selectedEntityId || null;
          const result = {
            flightCount: state?.flightCount || 0,
            trackedAtFlight: state?.trackedAtFlight,
            trackingReleased: mashaer.viewer.trackedEntity == null,
            generationBefore: state?.generationBefore,
            generationAfter: mashaer.styleManager._navigationGeneration,
          ...
          if (state) {
            mashaer.viewer.camera.flyToBoundingSphere = state.originalFly;
            window.removeEventListener('mashaer:world-request-focus', state.onRequest);
            if (state.sentinel) mashaer.viewer.entities.remove(state.sentinel);
          }
