...
test('tracked position source reads only mashaerDisplayPosition and never entity.position', () => {
...
test('the tracked card anchors to the visual position without repurposing the display accessor', () => {
  ...
  // model you can see, while `mashaerDisplayPosition` keeps returning the cached
  // dead-reckoned value the follow camera settled on (anti-jitter contract).
  const display = { x: 1, y: 2, z: 3 };
  const visual = { x: 1, y: 2, z: 103 };
  let displayReads = 0;
  const entity = {
    mashaerTrackedId: 'flights:aaa077',
    mashaerDisplayPosition: () => { displayReads += 1; return display; },
    mashaerVisualPosition: () => visual,
    mashaerLabelModel: { title: 'SWA143', details: ['ON GROUND'], accent: '#6be8ff' },
  };
  ...
  const spriteOnly = { mashaerDisplayPosition: () => display };
  assert.equal(cachedTrackedVisualPosition(spriteOnly), display);
  assert.equal(cachedTrackedVisualPosition({ ...spriteOnly, mashaerVisualPosition: () => null }), display,
    'a null visual position falls back rather than blanking the card');
  assert.equal(
    cachedTrackedVisualPosition({
      mashaerDisplayPosition: () => display,
      mashaerVisualPosition: () => { throw new Error('model gone'); },
    }),
    display,
...
test('tracked entry factory pins the production protected-lane policy', () => {
  ...
  const entry = createTrackedOverlayEntry({
    mashaerTrackedId: 'satellites:25544',
    mashaerDisplayPosition: () => display,
    mashaerLabelModel: { title: 'ISS', details: ['420 km · NORAD 25544'], accent: '#ffd84d' },
  });
...
test('tracked entity publishes a protected host entry backed by the frame cache', () => {
  ...
  const entity = {
    id: 'generated',
    mashaerTrackedId: 'flights:abc123',
    mashaerDisplayPosition: () => display,
    mashaerLabelModel: { title: 'UAL123', details: ['FL350 · 451 kts'], accent: '#39d0ff' },
  };
  ...
  try {
    ...
    entity.mashaerLabelModel = { ...entity.mashaerLabelModel, details: ['FL360 · 455 kts'] };
...
test('selection lifecycle ignores vessels, accepts installations, and clears without stale cards', () => {
  ...
  const recorder = makeHostRecorder();
  const installation = {
    mashaerTrackedId: 'installations:fort-test',
    mashaerDisplayPosition: () => ({ x: 1, y: 2, z: 3 }),
    mashaerLabelModel: { title: 'FORT TEST', details: ['AIRFIELD'], accent: '#5aa9ff' },
  };
  globalThis.window = fakeWindow;
  _setTrackedOverlayHostForTest(recorder.host);
  try {
    ...
    fakeWindow.dispatchEvent(new CustomEvent('mashaer:entity-selected', {
    ...
    fakeWindow.dispatchEvent(new CustomEvent('mashaer:entity-selection-cleared', {
...
test('tracking layers write mashaerLabelModel and expose only their cached display positions', async () => {
  ...
  for (const [name, source] of files) {
    assert.ok(source.includes('.mashaerLabelModel ='), `${name} writes the explicit model directly`);
    assert.ok(source.includes('.mashaerDisplayPosition ='), `${name} exposes a display-position cache`);
  }
  assert.ok(sources['flights.js'].includes('mashaerDisplayPosition = _trackedDisplayCached'));
  assert.ok(sources['militaryFlights.js'].includes('mashaerDisplayPosition = _trackedDisplayCached'));
  assert.ok(sources['satellites.js'].includes('mashaerDisplayPosition = _trackedDisplayCached'));
