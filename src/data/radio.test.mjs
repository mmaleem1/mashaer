...
test('Radio camera navigation yields to every live tracked-entity owner', () => {
  ...
  for (const mashaerTrackedId of ['flights:abc123', 'military:def456']) {
    const trackedEntity = { mashaerTrackedId };
    viewer.trackedEntity = trackedEntity;
    assert.equal(radioCameraNavigationAllowed(viewer), false, mashaerTrackedId);
