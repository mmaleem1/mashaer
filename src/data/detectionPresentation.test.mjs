...
test('detection owns and releases the Cockpit lifecycle listener', async () => {
  ...
  assert.match(source, /addEventListener\('mashaer:cockpit-mode-changed', _cockpitModeListener\)/);
  assert.match(source, /removeEventListener\('mashaer:cockpit-mode-changed', _cockpitModeListener\)/);
  assert.match(
    source,
