 ...
 * `setStack(..., { silent: true })` fires no 'mashaer:map-stack-changed' event,
...
export function createTeleGeographySubmarineCableLayer({
  ...
  function registerPickEntity(entity, info) {
    entity.__mashaerTeleGeography = info;
  ...
  function resolvePickRecord(picked) {
    ...
    if (primitive) {
      ...
      if (primitive.__mashaerTeleGeography) return primitive.__mashaerTeleGeography;
    ...
    if (entity) {
      ...
      if (entity.__mashaerTeleGeography) return entity.__mashaerTeleGeography;
  ...
  return {
    ...
    init(viewer) {
      ...
      if (!_mapStackListener && mapStackEventTarget?.addEventListener) {
        ...
        mapStackEventTarget.addEventListener('mashaer:map-stack-changed', _mapStackListener);
      }
    ...
    destroy(viewer) {
      ...
      if (_mapStackListener && mapStackEventTarget?.removeEventListener) {
        mapStackEventTarget.removeEventListener('mashaer:map-stack-changed', _mapStackListener);
