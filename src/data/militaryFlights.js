 ...
 * no `_modelDisplayPosition` from postRender, and `mashaerDisplayPosition` keeps its
...
function _startTrail(icao24) {
  ...
  if (!_trailHeadEntity && _viewer) {
    _trailHeadEntity = _viewer.entities.add({
      ...
      id: `mashaer-trail:mil-head-${++_trailHeadSeq}`,
...
function _trackFlight(icao24, { origin = 'programmatic' } = {}) {
  ...
  _trackedEntity.mashaerTrackedId = `military:${icao24}`;
  _trackedEntity.mashaerLabelModel = trackedLabelModelFromText(
  ...
  // Separate accessor on purpose (mirror of flights.js): `mashaerDisplayPosition` keeps the
  // follow-camera anti-jitter contract; presentation that must weld to the aircraft you
  // can see reads `mashaerVisualPosition`.
