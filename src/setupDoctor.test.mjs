...
test('doctor rejects an empty node_modules and requires every direct package', () => {
  const root = mkdtempSync(path.join(tmpdir(), 'mashaer-doctor-deps-'));
  try {
...
test('Pinokio-scoped diagnosis ignores Keychain items its start path does not import', () => {
  const root = mkdtempSync(path.join(tmpdir(), 'mashaer-pinokio-doctor-'));
  try {
...
test('Pinokio-scoped diagnosis does not count dotenv values shadowed by blank app fields', () => {
  const root = mkdtempSync(path.join(tmpdir(), 'mashaer-pinokio-doctor-env-'));
  try {
...
test('doctor reads the dotenv ladder without requiring Vite to be installed', () => {
  const root = mkdtempSync(path.join(tmpdir(), 'mashaer-doctor-'));
  try {
    writeFileSync(path.join(root, '.env'), 'MASHAER_TEST_KEY=base\n');
    writeFileSync(path.join(root, '.env.local'), 'MASHAER_TEST_KEY=local\n');
    writeFileSync(path.join(root, '.env.development.local'), 'MASHAER_TEST_KEY=mode-local\n');
    assert.equal(readDoctorDotenvValue('MASHAER_TEST_KEY', root), 'mode-local');
