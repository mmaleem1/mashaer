...
test('the boot provenance snapshot survives in-process Vite config re-evaluation', () => {
  ...
  assert.match(
    source,
    /const PROVIDER_ENV_AT_BOOT = globalThis\.__MASHAER_PROVIDER_ENV_AT_BOOT \?\?= Object\.freeze\(/,
  );
});
...
test('the sharing gate treats a real PINOKIO_SHARE_VAR as sharing, but not the empty/sentinel normal state', async () => {
  ...
  assert.equal(admitKeySetupRequest({ ...base, env: { PINOKIO_SHARE_VAR: '__mashaer_sharing_disabled__' } }).ok, true, 'the disabled sentinel is normal');
