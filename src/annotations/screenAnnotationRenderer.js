...
export function createScreenAnnotationRenderer(viewer, {
  ...
  function add(anno) {
    const c = color(anno);
    const group = svgEl('g', { class: 'mashaer-anno', opacity: '0' });
    const parts = {};
    ...
    if (anno.type === 'area' && anno.ring && anno.ring.length >= 3) {
      ...
      parts.poly = svgEl('polygon', {
        class: anno.synthesized ? 'mashaer-anno-area' : 'mashaer-anno-area mashaer-draw',
        fill: c, stroke: c,
        'fill-opacity': anno.synthesized ? '0.08' : '0.14',
        'stroke-width': '2.5',
        ...(anno.synthesized ? { 'stroke-dasharray': '9 7' } : {}),
        filter: 'url(#mashaer-sketch)',
      });
      ...
      parts.path = svgEl('path', {
        class: 'mashaer-anno-arrow mashaer-draw', fill: 'none', stroke: c,
        'stroke-width': '3', 'marker-end': `url(#mashaer-arrow-${anno.color || 'primary'})`,
        filter: 'url(#mashaer-sketch)',
      });
      ...
      parts.dots = anno.path.map(() => {
        const dot = svgEl('circle', { class: 'mashaer-anno-dot', fill: c, stroke: '#06121c', 'stroke-width': '2', r: '4' });
      ...
      if (anno.type !== 'label') {
        parts.ringOuter = svgEl('circle', { class: 'mashaer-anno-ring mashaer-pulse', fill: 'none', stroke: c, 'stroke-width': '2', r: '34' });
        parts.ringInner = svgEl('circle', { class: 'mashaer-anno-ring', fill: c, 'fill-opacity': '0.12', stroke: c, 'stroke-width': '2.5', r: '18', filter: 'url(#mashaer-sketch)' });
        group.appendChild(parts.ringOuter);
        group.appendChild(parts.ringInner);
      }
      parts.dot = svgEl('circle', { class: 'mashaer-anno-dot', fill: c, stroke: '#06121c', 'stroke-width': '2', r: anno.type === 'label' ? '4' : '5' });
      group.appendChild(parts.dot);
      parts.leader = svgEl('line', { class: 'mashaer-anno-leader', stroke: c, 'stroke-width': '1.5', 'stroke-opacity': '0.7' });
    ...
    try {
      ...
      requestAnimationFrame(() => group.classList.add('mashaer-in'));
      ...
      for (const drawEl of group.querySelectorAll('.mashaer-draw')) {
  ...
  function trackedEntityRect() {
    ...
    const world = typeof ent?.mashaerDisplayPosition === 'function'
      ? ent.mashaerDisplayPosition()
      : null;
    const win = world
  ...
  function projectAll() {
    ...
    for (const rec of records.values()) {
      ...
      // (The CSS `transition: opacity` was removed from .mashaer-anno so this per-frame JS ease
  ...
  function remove(anno) {
    const rec = records.get(anno.id);
    if (!rec) return;
    rec.group.classList.remove('mashaer-in');
    rec.group.classList.add('mashaer-out');
...
function buildOverlay() {
  ...
  layer.className = 'mashaer-screen-whiteboard';
  const svg = document.createElementNS(SVGNS, 'svg');
  svg.setAttribute('class', 'mashaer-screen-whiteboard-svg');
  svg.setAttribute('preserveAspectRatio', 'none');
  const defs = document.createElementNS(SVGNS, 'defs');
  // subtle hand-drawn wobble for strokes
  defs.innerHTML = `
    <filter id="mashaer-sketch" x="-20%" y="-20%" width="140%" height="140%">
...
function makeCallout(text, c) {
  ...
  node.setAttribute('class', 'mashaer-anno-callout');
  const rect = document.createElementNS(SVGNS, 'rect');
  rect.setAttribute('rx', '5');
  rect.setAttribute('class', 'mashaer-anno-card');
  const t = document.createElementNS(SVGNS, 'text');
  t.setAttribute('class', 'mashaer-anno-text');
  t.setAttribute('x', '9');
  t.setAttribute('y', '16');
  t.textContent = text;
  const accent = document.createElementNS(SVGNS, 'rect');
  accent.setAttribute('class', 'mashaer-anno-accent');
...
function ensureArrowMarker(defs, key, c) {
  if (defs.querySelector(`#mashaer-arrow-${key}`)) return;
  const marker = document.createElementNS(SVGNS, 'marker');
  marker.setAttribute('id', `mashaer-arrow-${key}`);
...
function injectStyles() {
  if (document.getElementById('mashaer-screen-whiteboard-styles')) return;
  const style = document.createElement('style');
  style.id = 'mashaer-screen-whiteboard-styles';
  style.textContent = `
  .mashaer-screen-whiteboard { position: fixed; inset: 0; pointer-events: none; z-index: 90; }
  .mashaer-screen-whiteboard-svg { position: absolute; inset: 0; width: 100%; height: 100%; overflow: visible; }
  /* No CSS opacity transition here on purpose: group opacity is driven per-frame in JS
     ...
     (flicker). Fade-OUT keeps its own transition via .mashaer-anno.mashaer-out below. */
  .mashaer-anno-ring { filter: drop-shadow(0 0 6px currentColor); }
  .mashaer-anno-dot { filter: drop-shadow(0 0 5px rgba(255,255,255,0.6)); }
  .mashaer-anno-area { filter: drop-shadow(0 0 5px currentColor); }
  .mashaer-anno-arrow { filter: drop-shadow(0 0 4px currentColor); stroke-linecap: round; }
  .mashaer-anno-leader { stroke-dasharray: 2 3; }
  .mashaer-anno-card { fill: rgba(8,18,28,0.78); stroke: rgba(255,255,255,0.14); stroke-width: 1; }
  .mashaer-anno-text { fill: #eaf6ff; font: 600 13px "JetBrains Mono", ui-monospace, monospace; letter-spacing: 0.02em; }
  /* draw-on: outlined shapes reveal their stroke */
  .mashaer-draw { stroke-dasharray: 1400; stroke-dashoffset: 1400; }
  .mashaer-anno.mashaer-in .mashaer-draw { transition: stroke-dashoffset 900ms ease-out; stroke-dashoffset: 0; }
  ...
  .mashaer-anno-callout { opacity: 0; }
  .mashaer-anno.mashaer-in .mashaer-anno-callout { animation: mashaer-pop 320ms ease-out both; }
  .mashaer-anno.mashaer-out { opacity: 0 !important; transition: opacity 320ms ease; }
  .mashaer-pulse { animation: mashaer-ring-pulse 1.8s ease-in-out infinite; transform-box: fill-box; transform-origin: center; }
  @keyframes mashaer-ring-pulse { 0%,100% { opacity: 0.9; stroke-width: 2; } 50% { opacity: 0.35; stroke-width: 3.5; } }
  @keyframes mashaer-pop { from { opacity: 0; } to { opacity: 1; } }
  `;
