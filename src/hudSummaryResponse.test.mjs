...
test('the installed keyless HUD route stays successful after the voice quota is exhausted', async () => {
  ...
  const previousLimit = process.env.MASHAER_RATELIMIT_OPENAI_PER_MIN;
  process.env.OPENAI_API_KEY = '';
  process.env.MASHAER_RATELIMIT_OPENAI_PER_MIN = '1';
  try {
    ...
    if (previousLimit === undefined) delete process.env.MASHAER_RATELIMIT_OPENAI_PER_MIN;
    else process.env.MASHAER_RATELIMIT_OPENAI_PER_MIN = previousLimit;
  }
});
