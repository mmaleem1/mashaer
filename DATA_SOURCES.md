...
Mashaer's **code** is [MIT](LICENSE)-licensed. **The MIT grant covers the source code only — it does NOT extend to third-party data or visual assets.** Every third-party source keeps its own license and terms. This file documents the live and bundled data sources; bundled 3D-model provenance is recorded in [`public/models/README.md`](public/models/README.md).
...
| **Makkah / Madinah camera presets** (30 POIs, 6 presets) | `src/data/makkahPresets.generated.js` (+ `scripts/fixtures/makkah-geocode-cache.json`) | **ODbL 1.0** (OpenStreetMap/Nominatim) + **CC BY 4.0** (Open-Meteo elevation) | ✅ (attribution; see note) | "© OpenStreetMap contributors" + "Elevation data by Open-Meteo.com" |
...
> If you use Mashaer commercially, delete `src/data/local_data/telegeography_submarine_cables/` (or obtain a commercial license from TeleGeography). It is one self-contained folder; the rest of the app runs without it.
...
### Makkah / Madinah camera presets (generated)

Unlike every other bundled dataset these are not a folder of geometry but a generated
JS module: `src/data/makkahPresets.generated.js`, built by
`scripts/build-makkah-presets.mjs` from the place list in `scripts/makkah-places.json`
(`npm run build:makkah-presets`).

Two upstream sources, both queried at BUILD time only - the shipped app makes no
request to either, and no key is involved:

- **Coordinates and feature extents** come from the OpenStreetMap **Nominatim** search
  API, each query bounded to a Makkah or Madinah viewbox. OSM data is **ODbL 1.0**, so
  the credit "© OpenStreetMap contributors" is required wherever these presets are
  shown. Every lookup is recorded in `scripts/fixtures/makkah-geocode-cache.json`
  (committed) with the resolving `osm_type/osm_id`, which is both the build cache and
  the provenance record for each coordinate.
- **Ground elevations** come from **Open-Meteo**'s elevation API, whose data licence is
  **CC BY 4.0** and requires the adjacent credit "Elevation data by Open-Meteo.com".

On ODbL share-alike: what ships is a small set of individual facts (a name, a point, a
bounding box) used to aim a camera, not a Derivative Database, and it is well under the
"substantial" threshold that triggers the share-alike obligation. The attribution is
given regardless. If these presets ever grow into a bulk OSM extract, they move into the
same share-alike bucket as `datacenters/` and `dams/` - see the ODbL note above.
